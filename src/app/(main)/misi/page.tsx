...
// PERBAIKAN: Hapus 'doc' yang tidak digunakan
import { collection, query, onSnapshot, orderBy } from 'firebase/firestore';
import { getFunctions, httpsCallable } from 'firebase/functions';
import MissionCard from '@/components/MissionCard';
import Button from '@/components/Button';
import { motion } from 'framer-motion';
// PERBAIKAN: Hapus 'Lock' yang tidak digunakan
...
export default function MissionsPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // PERBAIKAN: Gunakan onSnapshot pada doc untuk mendapatkan profil
      const profileRef = collection(db, 'users'); // Referensi ke koleksi
      const unsubProfile = onSnapshot(query(collection(profileRef, user.uid, 'profile')), (snap) => {
          if (!snap.empty) {
              setUserXP(snap.docs[0].data()?.xp || 0);
          }
      });
